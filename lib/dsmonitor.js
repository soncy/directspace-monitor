...
DSMonitor.prototype.sendEmail = function(config) {
    ...
    for(var i = 0, len = emails.length; i < len; i++) {
        var email = emails[i];
        self._sendEmail(email, config);
        log(nowDate() + ':放货了，已发送邮件到:' + email);
    }
}

DSMonitor.prototype._sendEmail = function(email, config) {
    var self   = this,
        c      = self.config,
        config = config || {};
    ...
    nodemailer.send_mail({
        ...
        to: email,
        subject: 'DirectSpace 有货啦！' + (config.test ? '(测试，请忽略)' : ''),
